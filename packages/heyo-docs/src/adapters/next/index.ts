...
import { loadOpenApiDocuments } from "../../server";
...
const NEXT_MDX_PLUGIN = fileURLToPath(new URL("./plugins.js", import.meta.url));
...
export async function generateNextContent({
  ...
  await Promise.all([
    ...
    writeFile(
      ...
      themeStylesheet(
        config.theme,
        themeSourceDirectories(root, contentDirectory),
      ),
    ),
  ]);
}
...
function clientModule(
  ...
  // Only AI provider credentials are server-only. The rest of the validated
  // configuration is declarative and safe to embed in the browser module.
...
export { default as remarkHeyoDocsAssets } from "./plugins";
