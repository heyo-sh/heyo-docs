...
export function heyoDocs(options: HeyoDocsViteOptions): HeyoDocsVitePlugin {
  ...
  let themeStylesheetPath = resolve(
    root,
    "node_modules",
    ".heyo-docs",
    "theme.css",
  ...
  const writeThemeStylesheet = () => {
    mkdirSync(dirname(themeStylesheetPath), { recursive: true });
    writeFileSync(
      themeStylesheetPath,
      themeStylesheet(
  ...
  return {
    ...
    resolveId(id) {
      if (id === THEME_CSS_MODULE) return themeStylesheetPath;
      if (
        id === CONTENT_MODULE ||
...
function createContentModule(
  ...
  const modules = pages
    .map((page, index) => {
      ...
      // producing HTML. The browser receives the same page metadata with a
      // React.lazy component, which makes every MDX document its own chunk.
      return isServer
        ? `import Page${index} from ${source};`
        : `const Page${index} = lazy(() => import(${source}));`;
    })
    .join("\n");
  const pageData = pages
    .map((page, index) => serializePage(page, index, config))
    .join(",\n");
  const lazyImport = isServer ? "" : 'import { lazy } from "react";\n\n';
  return `${lazyImport}${modules}\n\nexport const pages = [\n${pageData}\n];\n`;
}
