...
function HighlightedCode({
  ...
  // Keep first paint and SSR small. Shiki and its grammars are fetched only by
  // pages that actually contain a supported code fence.
  const [html, setHtml] = useState(() => plainCodeHtml(source));
  useEffect(() => {
    let active = true;
    void import("./mdx/highlighter").then(({ highlightCode }) => {
      if (active) setHtml(highlightCode(source, language));
    });
    return () => {
      active = false;
    };
