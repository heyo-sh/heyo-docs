...
`Button` uses the same shadcn component as the theme. It accepts `variant` and `size`, and uses `href` when the button should navigate. Use `align="left"`, `"center"`, or `"right"` to position it in the documentation column. Available variants are `primary`, `secondary`, `outline`, `ghost`, `destructive`, `link`, and `icon`; sizes are `xs`, `sm`, `default`, and `lg`.
